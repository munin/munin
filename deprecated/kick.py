...
class kick(loadable.loadable):
    ...
    def execute(self, nick, username, host, target, prefix, command, user, access):
        ...
        if res["success"]:
            reply = (
                "You have kickvoted '%s', who now has %s of 7 votes necessary to kick"
                % (idiot.pnick, res["retmessage"])
            )
            self.client.reply(prefix, nick, target, reply)
        else:
            reply = "You may not kickvote '%s'. Reason: %s" % (
                idiot.pnick,
                res["retmessage"],
            )
            self.client.reply(prefix, nick, target, reply)
        ...
        if res["success"]:
            ...
            self.client.privmsg(
                "p",
                "ban #ascendancy *!*@%s.users.netgamers.org You have been kicked from Ascendancy for being unpopular"
                % (idiot.pnick,),
            )
            reply = "%s has been reduced to level 1 and removed from the channel" % (
                idiot.pnick,
            )
            self.client.reply(prefix, nick, target, reply)
        return 1
