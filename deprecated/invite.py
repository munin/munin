...
class invite(loadable.loadable):
    ...
    def execute(self, nick, username, host, target, prefix, command, user, access):
        ...
        gimp = m.group(1)
        sponsor_munin_number = u.munin_number(
            self.conn, self.client, self.cursor, self.config
        )
        invites = 0
        ...
        if res["success"]:
            self.client.privmsg(
                "P", "adduser #%s %s 399" % (self.config.get("Auth", "home"), gimp,)
            )
            self.client.privmsg(
                "P",
                "modinfo #%s automode %s op" % (self.config.get("Auth", "home"), gimp,),
            )
            reply = (
