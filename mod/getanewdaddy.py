...
class getanewdaddy(loadable.loadable):
    ...
    def execute(self,nick,username,host,target,prefix,command,user,access,irc_msg):
        ...
        if idiot.sponsor != voter.pnick:
            self.client.privmsg('p',"note send %s Some admin has removed you from %s for whatever reason. If you still wish to be a member, go ahead and find someone else to sponsor you back."%(idiot.pnick, self.config.get('Auth', 'alliance')))
