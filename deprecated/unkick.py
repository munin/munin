...
class unkick(loadable.loadable):
    ...
    def __init__(self, client, conn, cursor):
        ...
        self.helptext = [
            "This command is used to undo a vote for someone to be kicked out of the alliance. Your vote is removed and you instantly become a saint."
        ]
    ...
    def execute(self, nick, username, host, target, prefix, command, user, access):
        ...
        if res["success"]:
            reply = (
                "You have unkickvoted '%s', who now has %s of 7 votes necessary to kick"
                % (idiot.pnick, res["retmessage"])
            )
        else:
            reply = "You may not unkickvote '%s'. Reason: %s" % (
                idiot.pnick,
                res["retmessage"],
            )
