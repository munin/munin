...
class parser:
    def __init__(self, config, client, munin):
        ...
        self.commandre = re.compile(
            r"^(%s|%s|%s)(.*)\s*$" % (self.notprefix, self.pubprefix, self.privprefix)
        )
    ...
    def parse(self, line):
        ...
        if m:
            ...
            if self.config.has_option("IRC", "modes"):
                self.client.wline(
                    "MODE %s +%s" % (self.munin.nick, self.config.get("IRC", "modes"))
                )
            return None
        ...
        if m:
            ...
            self.cursor.execute(
                ...
                (
                    user,
                    target,
                    self.prefix_to_numeric(prefix) == self.client.NOTICE_PREFIX
                ...
                if m:
                    ...
                    if err_msg:
                        self.client.reply(
                            ...
                            nick,
                            target,
                            "Failed to load module '%s' with reason: '%s'"
                            % (mod_name, err_msg),
                        )
                    else:
                        self.client.reply(
                            ...
                            nick,
                            target,
                            "Successfully loaded module '%s'" % (mod_name,),
                        )
    ...
    def scan(self, rand_id, nick, pnick, group_id):
        s = scan.scan(
            rand_id, self.client, self.conn, self.cursor, nick, pnick, group_id
        )
        s.run()
        ...
        for k in self.ctrl_list.keys():
            ...
            try:
                if ctrl.execute(irc_msg.user, irc_msg.access, irc_msg):
                    return "Successfully executed command '%s' with key '%s'" % (
                ...
                irc_msg.reply(
                    "Error in module '"
                    + ctrl.__class__.__name__
                    + "'. Please report the command you used to jester as soon as possible."
                )
                print e.__str__()
                traceback.print_exc()
                if DEBUG:
                    ...
                    if err_msg:
                        irc_msg.reply(
                            "Unable to reload module '%s', this may seriously impede further use"
                            % (k,)
                        )
                        print err_msg
        return None
    ...
    def load_mod(self, mod_name):
        try:
            ...
            if mod_name == "galstatus":
                ...
                self.galstatus = galstatus.galstatus(
                    self.client, self.conn, self.cursor, self.config
                )
                return None
    ...
    def help(self, irc_msg, param):
            ...
            if self.ctrl_list.has_key(param):
                if irc_msg.access >= self.ctrl_list[param].level:
                    try:
                        ...
                        self.ctrl_list[param].help(
                            irc_msg.user, irc_msg.access, irc_msg
                        )
                        return "Successfully executed help for '%s' with key '%s'" % (
                            self.ctrl_list[param].__class__.__name__,
                            param,
                        )
                    except Exception, e:
        ...
        irc_msg.reply(
            ...
            + self.notprefix.replace("|", "")
            + self.pubprefix.replace("|", "")
            + self.privprefix.replace("|", "")
            + ">help <command>. Built-in commands: help"
            + (bool(irc_msg.access >= 1000) and ", loadmod" or "")
        )
        command_list = []
