...
class f(loadable.loadable):
    ...
    def __init__(self, cursor):
        ...
        self.usage_oeta = (
            self.__class__.__name__ + " <id> <oeta|launch> <original_eta|launch_tick>"
        )
        self.helptext = [
            "Show or modify the status of a fleet. Possible modifications include eta/land to modify landing tick, oeta/launch to modify original eta/launch tick or delete to delete the fleet."
        ]
    ...
    def cmd_eta(self, f, when):
        ...
        if self.cursor.rowcount < 1:
            irc_msg.reply(
                "Could not update ETA for fleet with id %s, no matching fleets found"
                % (f.id,)
            )
        else:
            irc_msg.reply(
                "Updated landing tick for fleet id %s from %s to %s (new eta is %s)"
                % (f.id, f.landing_tick, tick, eta)
            )
        return 1
    ...
    def cmd_orig_eta(self, f, when):
        ...
        if self.cursor.rowcount < 1:
            irc_msg.reply(
                "Could not update original ETA for fleet with id %s, no matching fleets found"
                % (f.id,)
            )
        else:
            irc_msg.reply(
                "Updated launch tick for fleet id %s from %s to %s (new original eta is %s)"
                % (f.id, f.launch_tick, tick, eta)
            )
        return 1
    ...
    def cmd_delete(self, f):
        ...
        if self.cursor.rowcount < 1:
            irc_msg.reply(
                "Something went wrong. Old status was %s, new status was %s, defcall id was %s"
                % (old_status, s["status"], d.id)
            )
        else:
            irc_msg.reply(
                "Updated defcall %s on %s:%s:%s landing pt %s from status '%s' to '%s'"
                % (d.id, p.x, p.y, p.z, d.landing_tick, d.actual_status, s["status"])
            )
