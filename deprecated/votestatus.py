...
class votestatus(loadable.loadable):
	...
	self.helptext=['This command either shows the list of nicks currently being voted to be kicked from the alliance or, if given a nick, it will show the people currently voting for that nick to be kicked (ie a list of cunts)']
    ...
    def execute(self,nick,username,host,target,prefix,command,user,access):
        ...
        if m:
            ...
            if not idiot.load_from_db(self.conn,self.client,self.cursor):
                self.client.reply(prefix,nick,target,"That idiot doesn't exist, if anyone wants to fix you being an idiot they can kickvote %s" % (user,))
                return 1
