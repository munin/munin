...
class phone(loadable.loadable):
    ...
    def execute(self,nick,host,target,prefix,command,user,access,irc_msg):
        ...
        if "allow".find(command) > -1:
            results=self.phone_query_builder(nick,host,target,prefix,command,u,access,"AND t1.friend_id=%s",(t_user.id,))
            if len(results) > 0:
            ...
            if t_user.pubphone and u.userlevel >= 100:
                ...
                results=self.phone_query_builder(nick,host,target,prefix,command,t_user,access,"AND t1.friend_id=%s",(u.id,))
                if len(results) < 1:
