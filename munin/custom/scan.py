...
class scan(threading.Thread):
    def __init__(self, rand_id, client, config, nick, pnick, group_id):  # random scan ID, and client for debug ONLY
        ...
        self.useragent = "Munin (Python-urllib/%s); BotNick/%s; Admin/%s" % (
            urllib.request.__version__,config.get("Connection", "nick"), config.get("Auth", "owner_nick"))
    ...
    def parse_development(self, scan_id, page):
        ...
        args = ()
        args += (scan_id, lightfactory, medfactory, heavyfactory, waveamp, wavedist, metalref,
                 crystalref, eref, reslab, finance, military, security, structuredef)
