...
# Nothing ascendancy specific here as far as I can tell.
# qebab, 24/6/08
...
class sponsor(loadable.loadable):
    ...
    def execute(self, nick, username, host, target, prefix, command, user, access):
        ...
        if recruit.lower() == "kila".lower():
            reply = (
                "You have successfully invited '%s'. The gimp is now your responsibility. If they fuck up and didn't know, it's your fault. So teach them well."
                % (recruit,)
            )
        ...
        if res["success"]:
            reply = (
                "You have sponsored '%s' (MAKE SURE THIS IS THE RECRUIT'S PNICK.) When you have ensured that there are no objections you may use the !invite command to make them a member."
                % (recruit,)
            )
        else:
            reply = "You may not sponsor '%s'. Reason: %s" % (
                recruit,
                res["retmessage"],
            )
        self.client.reply(prefix, nick, target, reply)
